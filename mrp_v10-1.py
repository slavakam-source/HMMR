...
def _gp_del_formula(gp_ri, di):
    ...
    if di + 1 < len(all_dates):
        _, mnext, day_next = all_dates[di + 1]
        dem_next = _gp_demand_ref(gp_ri, mnext, day_next)
    else:
        dem_next = "0"
    proj = f"({prev_ss}-{dem_today})"
    target = f"MAX(F{gp_ri},CEILING({DELIVERY_START_COVER}*({dem_next}),1))"
    return (
        f"=IF({proj}<{target},"
        f"CEILING(MAX(0,{target}-{proj})/E{gp_ri},1)*E{gp_ri},0)"
    )
